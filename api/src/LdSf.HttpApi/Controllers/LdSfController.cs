...
public class LdSfController : AbpControllerBase
{
    ...
    [HttpPost("auth/login")]
    public Task<LoginDto> LoginAsync(LoginInput input) => _service.LoginAsync(input);
    ...
    public Task<SubaccountDto> CreateSubaccountAsync(CreateSubaccountInput input) => _service.CreateSubaccountAsync(input);

    [HttpPost("admin/subaccounts/{subaccountId:guid}/balance")]
    public Task<SubaccountDto> UpdateBalanceAsync(Guid subaccountId, UpdateBalanceInput input) => _service.UpdateBalanceAsync(subaccountId, input);

    [HttpPost("admin/authorizations/{identifier}/grant")]
    public Task<SubaccountDto> GrantUsesAsync(string identifier, GrantUsesInput input) => _service.GrantUsesAsync(identifier, input);
    ...
    public Task<SmsTaskDto> CreateTaskAsync(Guid subaccountId, CreateTaskInput input) => _service.CreateTaskAsync(subaccountId, input);
    ...
    public Task<AuthorizationDto> AuthorizeAsync(AuthorizeInput input) => _service.AuthorizeAsync(input);
    ...
    public Task ConfirmSentAsync(Guid taskId, ConfirmSentInput input) => _service.ConfirmSentAsync(taskId, input);
}
